...
class AppStorage {
    ...
    private init() {
        ...
        do {
            // 使用自訂建構器創建 FuseStorageKit - 注意：AppDatabase 內部已經創建了 FuseStorageKit
            // 保留這個實例是為了直接使用 FuseStorageKit 的文件和偏好設定操作
            storageKit = try FuseStorageKitBuilder()
                .with(databaseManager: FuseDatabaseManager(path: "fuse.sqlite"))
                .with(fileManager: FuseFileManager.withBaseFolder("NoteAttachments"))
                .build()
    ...
    var database: FuseDatabaseManageable {
        return storageKit.databaseManager
    }
    ...
    func saveNote(_ note: Note, image: UIImage? = nil) throws {
        ...
        if let image = image {
            ...
            _ = try storageKit.saveImage(image, relativePath: imagePath)
        ...
        // 儲存或更新筆記 - 使用 database 實例
        try database.insert(noteToSave)
    }
    ...
    func getAllNotes(withTitle title: String? = nil) throws -> [Note] {
        return try database.read { db in
            if let title = title, !title.isEmpty {
                // 使用 Column 物件進行模糊搜尋
                let titleColumn = Column(Note.Field.title)
                return try Note.filter(titleColumn.like("%\(title)%")).fetchAll(db)
            } else {
                // 使用 Column 物件進行排序
                let createdAtColumn = Column(Note.Field.createdAt)
                return try Note.order(createdAtColumn.desc).fetchAll(db)
            }
        }
    }
    ...
    func getNote(withID id: String) throws -> Note? {
        return try database.read { db in
            let idColumn = Column(Note.Field.id)
            return try Note.filter(idColumn == id).fetchOne(db)
        }
    }
    ...
    func deleteNote(_ note: Note) throws {
        ...
        if note.hasAttachment, let path = note.attachmentPath {
            try storageKit.deleteFile(at: path)
        }
    }
    ...
    func getAttachmentImage(for note: Note) -> UIImage? {
        ...
        let url = storageKit.getFileURL(for: path)
    ...
    func saveThemePreference(isDarkMode: Bool) {
        storageKit.setPreference(isDarkMode, forKey: "isDarkMode")
    }
    
    func getThemePreference() -> Bool {
        storageKit.getPreference(forKey: "isDarkMode") ?? false
    }
