...
Then add the appropriate target to your target dependencies:

**For standard SQLite databases:**
```swift
.target(
    name: "YourTarget",
    dependencies: [
...
**For encrypted databases with SQLCipher:**
```swift
.target(
    name: "YourTarget", 
    dependencies: [
        .product(name: "FuseStorageKitSQLCipher", package: "FuseStorageKit")
    ]
)
```
...
.target(
    ...
    dependencies: [
        .product(name: "FuseStorageKit", package: "FuseStorageKit"),
        // or .product(name: "FuseStorageKitSQLCipher", package: "FuseStorageKit"),
...
3. Choose the appropriate library:
   - `FuseStorageKit` for standard SQLite databases
   - `FuseStorageKitSQLCipher` for encrypted databases
...
// or import FuseStorageKitSQLCipher
...
#### Database Encryption (FuseStorageKitSQLCipher only)

**Note**: Database encryption is only available when using `FuseStorageKitSQLCipher`. It provides enterprise-grade AES-256 encryption via SQLCipher.

```swift
import FuseStorageKitSQLCipher
...
// Encrypted SQLite database (FuseStorageKitSQLCipher only)
...
- **GRDB.swift**: Database functionality via [groue/GRDB.swift](https://github.com/groue/GRDB.swift) (for FuseStorageKit)
- **GRDB.swift with SQLCipher**: Database encryption via [duckduckgo/GRDB.swift](https://github.com/duckduckgo/GRDB.swift) (for FuseStorageKitSQLCipher)
- **Firebase iOS SDK**: Sync functionality via [firebase/firebase-ios-sdk](https://github.com/firebase/firebase-ios-sdk) (optional)

### Library Comparison

| Feature | FuseStorageKit | FuseStorageKitSQLCipher |
|---------|----------------|-------------------------|
| Standard SQLite | ✅ | ✅ |
| Database Encryption | ❌ | ✅ (AES-256) |
| Performance | High | High |
| File Size | Smaller | Larger (includes SQLCipher) |
| Use Case | General purpose | Security-focused |

## Limitations

- Database encryption is only available in `FuseStorageKitSQLCipher`
